...
def _to_spatial_node(e) -> Dict[str, Any]:
    ...
    # Inline what used to be _spatial_children(...)
    kids: List[Any] = []
    try:
...
class IFCQuery:
    ...
    def resolve_ifc_spatial_hierarchy(_parent, _info, filePath: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Preferred route: Sites from the first Project
            ...
            # Fallbacks for minimal/atypical models
            if not roots:
    ...
    # ---------- 3D clash detection (exact OCC) ----------
    @staticmethod
    def resolve_detect_clashes(_parent, _info, filePath: str) -> List[Dict[str, Any]]:
        """
        detectClashes(filePath: String!): [ClashResult]
        Returns rows { element1, element2, intersectionVolume } for pairs with volume > 0.
        Types scanned are limited for speed; adjust 'types' list as needed.
        """
        if not os.path.isfile(filePath):
            raise GraphQLError(f"File not found: {filePath}")
        ...
        try:
            ...
            # Control search space for performance (tune as needed)
            types = ["IfcWall", "IfcSlab", "IfcBeam", "IfcColumn", "IfcFooting", "IfcStair"]

            # Collect unique element GUIDs
            guids: List[str] = []
            seen: set[str] = set()
            for t in types:
                for e in m.by_type(t):
    ...
    # ---------- Optional: single-pair helpers ----------
    @staticmethod
    def resolve_pairwise_clash(_parent, _info, filePath: str, a: str, b: str) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def resolve_pair_clash_with_geometry(_parent, _info, filePath: str, a: str, b: str) -> Dict[str, Any]:
        """
        pairClashWithGeometry(filePath: String!, a: ID!, b: ID!): PairClashPayload
        Convenience: returns the clash volume plus minimal GLB info for A and B.
        """
        if not os.path.isfile(filePath):
            raise GraphQLError(f"File not found: {filePath}")
        try:
